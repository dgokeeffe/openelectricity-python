...
from openelectricity.types import NetworkCode, DataInterval
...
def benchmark_to_records(response: TimeSeriesResponse, iterations: int = 5) -> dict[str, float]:
    ...
    print(f"Benchmarking to_records with {len(response.data[0].results)} result groups and {len(response.data[0].results[0].data)} data points per group...")

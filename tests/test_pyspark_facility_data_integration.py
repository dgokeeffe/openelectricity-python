...
class TestPySparkFacilityDataIntegration:
    ...
    @pytest.mark.skipif(
        not pytest.importorskip("pyspark", reason="PySpark not available"),
    ...
    def test_timezone_conversion(self, client, test_parameters):
        ...
        assert spark_dt == expected_utc, \
            f"Timezone conversion failed: {spark_dt} != {expected_utc}"
    ...
    def test_temporal_operations(self, client, test_parameters):
        ...
        assert all(0 <= h <= 23 for h in hour_values), \
            f"Invalid hour values: {hour_values}"
        
        # Test date formatting
        formatted_df = spark_df.select(
            date_format('interval', 'yyyy-MM-dd HH:mm:ss').alias('formatted')
        )
        ...
        min_max_df = spark_df.select(
            spark_min('interval').alias('min_time'),
            spark_max('interval').alias('max_time')
        ).collect()
    ...
    def test_numeric_operations(self, client, test_parameters):
        ...
        if stats_df:
            ...
            assert isinstance(stats['min_val'], (int, float, type(None))), \
                f"Min value not numeric: {type(stats['min_val'])}"
            assert isinstance(stats['max_val'], (int, float, type(None))), \
                f"Max value not numeric: {type(stats['max_val'])}"
            assert isinstance(stats['avg_val'], (int, float, type(None))), \
                f"Avg value not numeric: {type(stats['avg_val'])}"
            assert isinstance(stats['sum_val'], (int, float, type(None))), \
                f"Sum value not numeric: {type(stats['sum_val'])}"
            assert isinstance(stats['count_val'], int), \
                f"Count not integer: {type(stats['count_val'])}"
